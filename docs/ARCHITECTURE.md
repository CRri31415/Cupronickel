...
| 메모 모듈(폴더 분류) | 구현 |
| 그래프 다이어그램 모듈(다중 보드 + 물리 + 노드/간선 편집·삭제 + PNG) | 구현 |
| 암기 모듈(덱 분류 + FSRS + 덱별 복습 + due 카운트 + 다음 복습일) | 구현 |
| 달력 모듈(중요도/태그/기간/후속·필터·히트맵, 즉시 반영) | 구현 |
| 노트 모듈(폴더 트리 + 전용 마크다운 + 편집/미리보기 + 문서간 링크) | 구현 |
| 텍스트 범용 확장 모듈(TeX/SMILES, 레지스트리 연동) | 구현 |
| 개발용 에디터 모듈(프로젝트 + vi 키맵 일부 + 줄번호 + 빌드/실행) | 구현 |
| 시트 모듈(논문 양식 표 + PNG 내보내기) | 구현 |
| 위젯 설정(추가/설정/삭제) | 구현 |
| 분할화면(이동 방식), keep-alive 상태 보존, 부착 메모 드래그 | 구현 |
| **7개 기능 모듈 전체 + 텍스트 확장 구현 완료** | ✓ |
